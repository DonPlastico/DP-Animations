...
Load.PtfxCreation = function(ped, prop, name, asset, placement, rgb)
    ...
    local newPtfx = StartNetworkedParticleFxLoopedOnEntityBone(name, ptfxSpawn, placement[1] + 0.0, placement[2] + 0.0,
        placement[3] + 0.0, placement[4] + 0.0, placement[5] + 0.0, placement[6] + 0.0,
        GetEntityBoneIndexByName(name, "VFX"), placement[7] + 0.0, 0, 0, 0, 1065353216, 1065353216, 1065353216, 0)
    if newPtfx then
...
---Elimina los efectos de partículas existentes
...
---Crea un accesorio en la ubicación
---@param ped number
---@param prop string
---@param bone number
---@param placement table
Load.PropCreation = function(ped, prop, bone, placement)
    ...
    if newProp then
        AttachEntityToEntity(newProp, ped, GetPedBoneIndex(ped, bone), placement[1] + 0.0, placement[2] + 0.0,
            placement[3] + 0.0, placement[4] + 0.0, placement[5] + 0.0, placement[6] + 0.0, true, true, false, true, 1,
            true)
...
Load.PropRemoval = function(type)
    if type == 'global' then
        if not cfg.propActive then
            ...
            QBCore.Functions.Notify('Impedido supresión real de prop...', 'info', 5000)
        end
    else
...
--- Obtiene el ped más cercano por raycast
---@return any
...
--- Envía la confirmación al jugador
---@param target number
---@param shared string
Load.Confirmation = function(target, shared)
    QBCore.Functions.Notify('[E] Accept Request\n[L] Deny Request', 'info', 5000)
        ...
        while not hasResolved do
            if IsControlJustPressed(0, cfg.acceptKey) then
                if not hasResolved then
                    ...
                    TriggerServerEvent('DP-Animations:resolveAnimation', target, shared, true)
                    hasResolved = true
                end
...
---Cancela las animaciones que se están reproduciendo actualmente
Load.Cancel = function()
    ...
    if cfg.ptfxActive then
        if cfg.ptfxOwner then
            TriggerServerEvent('DP-Animations:syncRemoval')
            cfg.ptfxOwner = false
        end
        Load.PtfxRemoval()
...
CreateThread(function()
    TriggerEvent('chat:addSuggestions', {{
        name = '/' .. cfg.commandNameEmote,
        help = cfg.commandNameSuggestion,
        params = {{
            name = 'emote',
            help = 'Nombre del gesto'
        }}
    }, {
        name = '/' .. cfg.commandName,
        help = cfg.commandSuggestion,
        params = {}
    }})
end)
